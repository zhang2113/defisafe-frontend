...
<script>
import { DEFISAFE_CONSTRACT, MINE_CONTRACT, CURRENT_NET } from "../constants";
...
export default {
  ...
  methods: {
    getData() {
      ...
      let mineContract = this.$util.getContract(
        this.web3Obj,
        MINE_CONTRACT[CURRENT_NET],
        MINE_CONTRACT.abi
      );
      ...
      mineContract.methods
        ...
        .call()
        .then((res) => {
          this.displayDES = parseFloat(this.web3Obj.utils.fromWei(res)).toFixed(
            4
          );
        });
    ...
    getContractDataCall(contract, methodsArr, accountInfo, index = 0) {
      ...
      contract.methods[methodsArr[index]]()
        .call()
        .then((res) => {
          if (option.name == "insureTimeEver") {
            accountInfo[option.name] = res;
          } else {
            accountInfo[option.name] = parseFloat(
              this.web3Obj.utils.fromWei(res)
            ).toFixed(4);
          }
